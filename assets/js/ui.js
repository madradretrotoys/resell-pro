...
// Global helper: assign color roles to a group of .btn buttons.
// - First actionable button => blue (btn-primary)
// - Remaining buttons => ghost (btn-ghost)
// - Respects any explicit color already present (won't override).
export function applyButtonGroupColors(root, options = {}) {
  ...
  if (allGhost) {
    btns.forEach((btn) => {
      if (!hasColorRole(btn)) btn.classList.add('btn-ghost');
    });
    return;
  }
  ...
  btns.forEach((btn) => {
    ...
    if (!madePrimary) {
      btn.classList.add('btn-primary'); // first actionable = blue
      madePrimary = true;
    } else {
      btn.classList.add('btn-ghost');   // neighbors = ghost
    }
  });
}
