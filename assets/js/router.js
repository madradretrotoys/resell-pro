...
async function loadHTML(url){ const r = await fetch(url, { credentials:'include' }); if(!r.ok) throw new Error(url); return r.text(); }
...
async function loadScreen(name){
  ...
  try { view.innerHTML = await loadHTML(meta.html + `?v=${Date.now()}`); }
  catch(e){ view.innerHTML = `<div class="error">Failed to load screen.</div>`; return; }

  try {
    const mod = await import(meta.js + `?v=${Date.now()}`);
    if(mod?.init) await mod.init({ container:view, session });
    current = { name, mod };
  } catch (e) { console.error(e); showToast('Screen script error'); }
...
function goto(name){
  const u = new URL(location.href); u.searchParams.set('page', name);
  history.pushState({}, '', u); loadScreen(name);
}

window.addEventListener('popstate', () => loadScreen(qs('page') || 'dashboard'));
document.addEventListener('click', (e) => {
  const a = e.target.closest('[data-page]'); if(!a) return;
  e.preventDefault(); goto(a.getAttribute('data-page'));
});
