*** /dev/null
--- a/functions/api/marketplaces/ebay/policies.ts
@@
...
+    const sql = neon(url);
+
+    // ---- AuthZ (mirror Intake create/edit) ----
...
+
+    // ---- Fetch eBay access details for this tenant ----
+    // We bind by slug='ebay' to find the marketplace id, then read the connection row.
+    const rows = await sql<{ access_token: string | null; refresh_token: string | null; environment: string | null }[]>`
+      SELECT mc.access_token, mc.refresh_token, mc.environment
...
+
+    // ---- Helper to fetch a policy collection ----
+    async function getList<T>(path: string, key: string): Promise<Array<{ id: string; name: string }>> {
...
+      // Normalize: eBay returns e.g. { fulfillmentPolicies: [{fulfillmentPolicyId, name}] }
+      const arr = Array.isArray(json?.[key]) ? json[key] : [];
+      return arr.map((p: any) => ({ id: String(p?.[Object.keys(p).find(k => /Id$/.test(k))!] || ""), name: String(p?.name || "") }))
