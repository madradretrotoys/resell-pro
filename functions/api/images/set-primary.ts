// /functions/api/images/set-primary.ts
...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  try {
    ...
    // scope check: ensure image belongs to tenant + item
    const owns = await sql<{ n:string }[]>`
      select count(*)::text as n from app.item_images
      where image_id=${image_id} and item_id=${item_id} and tenant_id=${tenant_id}
    `;
    if (Number(owns[0].n)===0) return json({ ok:false, error:"not_found" }, 404);

    await sql/*sql*/`update app.item_images set is_primary=false where item_id=${item_id}`;
    await sql/*sql*/`update app.item_images set is_primary=true  where image_id=${image_id}`;
