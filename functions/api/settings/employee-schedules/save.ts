...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  try {
    ...
    const settings = await sql/*sql*/`
      ...
      LIMIT 1
    `;
    const consecutive_lunch_hours_required = Number(settings?.[0]?.consecutive_lunch_hours_required ?? 0);
    const default_lunch_minutes = Number(settings?.[0]?.default_lunch_minutes ?? 30);
    const durationHours = durationMinutes / 60;
    if (consecutive_lunch_hours_required > 0 && durationHours > consecutive_lunch_hours_required && break_minutes <= 0) {
      return json({
        ok: false,
        error: "lunch_required",
        required_after_hours: consecutive_lunch_hours_required,
        suggested_lunch_minutes: default_lunch_minutes,
      }, 400);
    }
