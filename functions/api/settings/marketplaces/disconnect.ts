...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  try {
    ...
    const marketplace_id = Number(url.searchParams.get("marketplace_id") || 0);
    ...
    await sql/*sql*/`
      INSERT INTO app.marketplace_connections (tenant_id, marketplace_id, status)
      VALUES (${tenant_id}, ${marketplace_id}, 'disconnected')
      ON CONFLICT (tenant_id, marketplace_id)
      DO UPDATE SET status = 'disconnected', updated_at = now()
    `;

    return json({ ok: true, marketplace_id, status: "disconnected" });
  } catch (e: any) {
