...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  try {
    ...
    const current_tenant_id = request.headers.get("x-tenant-id");
    if (!current_tenant_id) return json({ ok: false, error: "missing_tenant" }, 400);

    const sql = neon(String(env.DATABASE_URL));
    const actor = await getTenantActor(sql, current_tenant_id, auth.actor_user_id);
