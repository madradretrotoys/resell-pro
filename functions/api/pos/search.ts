...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    // Platform/internal users still need explicit feature permissions.
    const actor = await getEffectiveTenantActor(sql, tenant_id, actor_user_id);
    console.log("[pos.search] auth", actor || null);
    if (!actor || actor.active === false) return json({ ok: false, error: "forbidden" }, 403);
    const allow = isPlatformScopedActor(actor)
      ? (!!actor.can_pos || !!actor.can_inventory_intake)
      : (["owner", "admin", "manager"].includes(actor.role) || !!actor.can_pos || !!actor.can_inventory_intake);
