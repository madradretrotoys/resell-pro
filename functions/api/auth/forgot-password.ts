...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  ...
  // Create a reset token (30 min expiry)
...
<p>If you did not request this, you can ignore this email.</p>`;
  ...
  await sendMail(env, user.email, subject, text, html).catch(() => { /* swallow to avoid leaking */ });
...
async function sendMail(
  ...
  const fromEmail = (env.MAIL_FROM as string) || "madradretrotoys@gmail.com";
